...
export class BitflowAdapter implements DexAdapter {
  readonly dexId = "bitflow";
  readonly chainFamily = "stacks";

  canHandle(dexId: string, chainId: ChainId): boolean {
    return chainId === "stacks:mainnet" && dexId.toLowerCase().includes("bitflow");
  }
  ...
  decodeSwap(pool: TrackedPool, rawLog: unknown): SwapEvent | null {
    ...
    if (!log?.repr) return null;
    const decoded = decodeStacksSwapPrint(log.repr, "bitflow");
