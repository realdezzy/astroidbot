...
### Config
`ConfigManager` (`src/config.ts`) is a zod-validated singleton over `process.env`. Call `ConfigManager.load()` once at startup (already done in `bootstrap()`); everywhere else use `ConfigManager.getInstance()`. Adding a new env var means adding it to `envSchema` first — accessing it via `.config.FOO` won't typecheck otherwise. Booleans must use the `z.enum(["true","false","1","0"]).transform(...)` shape, **not** `z.coerce.boolean()`: `Boolean("false")` is `true`, so a coerced flag is enabled by the value meant to disable it. `ENABLED_CHAINS` decides which chains this deployment runs and **defaults to multichain** — `stacks:mainnet,base:mainnet,solana:mainnet`, one per execution family (see `Docs/chains.md`). Every chain in that default runs with no credentials beyond Postgres and Redis; a descriptor that prefers ERC-4337 falls back to EOA custody with a warning when `PIMLICO_API_KEY` is absent, rather than refusing to boot. Unknown ChainIds are still fatal at startup.
