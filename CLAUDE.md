...
`INDEXER_MODE` (`off`/`inline`/`standalone`) decides which process ingests; every process in a deployment must agree, and compose pins it rather than leaving it to an env file. Decide it through `src/services/indexer/mode.ts` (`shouldIngestInline()`, `assertStandaloneProcess()`), never by comparing the config string yourself. See `Docs/market-data.md`.
