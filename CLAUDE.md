...
### Config
`ConfigManager` (`src/config.ts`) is a zod-validated singleton over `process.env`. Call `ConfigManager.load()` once at startup (already done in `bootstrap()`); everywhere else use `ConfigManager.getInstance()`. Adding a new env var means adding it to `envSchema` first — accessing it via `.config.FOO` won't typecheck otherwise. Booleans must use the `z.enum(["true","false","1","0"]).transform(...)` shape, **not** `z.coerce.boolean()`: `Boolean("false")` is `true`, so a coerced flag is enabled by the value meant to disable it. `ENABLED_CHAINS` decides which chains this deployment runs and **defaults to multichain** — `stacks:mainnet,base:mainnet,robinhood:mainnet,solana:mainnet`, one per execution family (see `Docs/architecture/chains.md`). Every chain in that default runs with no credentials beyond Postgres and Redis; a descriptor that prefers ERC-4337 falls back to EOA custody with a warning when `PIMLICO_API_KEY` is absent, rather than refusing to boot. Unknown ChainIds are still fatal at startup.

The indexer's block-denominated safety margins are resolved **per chain**, not globally. `INDEXER_CONFIRMATIONS` and `INDEXER_INITIAL_LOOKBACK_BLOCKS` are floors; a descriptor that states `indexer.blockTimeSeconds` has them raised to whatever covers `INDEXER_CONFIRMATION_SECONDS` / `INDEXER_INITIAL_LOOKBACK_HOURS` (`settingsForChain` in `src/services/indexer/settings.ts`, applied in `IndexerService.init()`). Raised, never lowered — an explicit setting must not be quietly weakened by a derivation. This exists because block times here span two orders of magnitude: twelve blocks was 144 seconds of reorg protection on Ethereum and **1.2 seconds** on Robinhood, whose blocks are ~100ms. Adding a chain faster than a couple of seconds a block means stating its block time, or it inherits a margin that protects almost nothing.

Polling is the intended transport and streaming is not a drop-in alternative. Alchemy bills WebSocket subscriptions by bytes delivered (0.04 CU/byte), which for a product whose finest resolution is a 5-minute candle costs ~120x `eth_getLogs` for the same data — on Robinhood, block headers alone cost more than reading every swap. More importantly a stream has no cursor, so gap detection needs a block-height watermark anyway: streaming is a latency layer over polling, never a replacement for it. If you add a subscription, it may enqueue work but must never advance a cursor or write a candle.
...
### User settings — two layers
`TradeSettings` is **account-wide**, one row per `(userId, context)` and constrained as such. `ChainPreference` holds **per-chain overrides**, one row per `(userId, chainId)`, with every non-key column nullable so a row states only what it overrides. Read them through `resolveTradeSettings(userId, context, chainId)` (`src/services/tradeSettings.ts`) — never `findTradeSettings` directly on a path that knows its chain, or the chain's slippage is silently ignored.

The split exists because the single table was two tables wearing one name: risk readers looked rows up by `(userId, context)` with an unordered `findFirst` while the gas-sponsorship endpoint keyed them by `(userId, chain)` and created one when absent, with no constraint on either shape. Toggling sponsorship for a second chain inserted a duplicate `(userId, "personal")` row full of product defaults, after which which limits `RiskManager` enforced was up to the query planner. Keep exposure limits (`maxPositionPct`, `dailyLossLimit`) account-wide: per-chain copies would let a user on three chains take three times the position they capped.

### Chain-aware clients
The backend has been multichain for a while; the clients are where it leaks. Three rules the web and Telegram surfaces have to follow:

- **Never hardcode a chain, a native symbol, or an explorer URL.** The web reads `/api/chains` through the `useChains` hook (`web/src/hooks/useChains.ts`) and the bot through `chainPicker`/`activeChain`. Explorer links come from the API (`explorerTxUrlFor` in `walletChain.ts`), because a `txId` means nothing without its network — four separate components each composed a Hiro URL, so every non-Stacks trade linked to a 404.
- **Aggregate by `chainId:symbol`, never by bare symbol.** USDC on Base and USDC on Solana are different assets. This applies client-side too: the Portfolio page re-introduced the bug that `10db259` fixed in backend analytics.
- **Anything that spends from a wallet picks the chain first.** Wallet creation, imports, trades and limit orders are all chain-first; a form that lets you select wallets across chains and submits one token pair produces orders that cannot route.
