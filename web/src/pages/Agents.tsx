...
type StrategyType = "portfolio_rebalance" | "grid" | "dca" | "sniper" | "copy";
...
const STRATEGY_INFO: Record<StrategyType, StrategyInfo> = {
  portfolio_rebalance: {
    icon: Gauge, label: "Portfolio Rebalance", desc: "AI-driven weight-based portfolio rebalancing.",
    defaults: { rebalanceThreshold: 2, maxPositionPct: 25 },
    fields: [
  ...
  grid: {
    icon: Grid3X3, label: "Grid Market Making", desc: "Buy/sell at fixed price intervals.",
    defaults: { tokenPair: "STX/sUSDT", levels: 5, spreadBps: 30, maxPositionPct: 25 },
    fields: [
      { key: "tokenPair", label: "Token Pair", type: "text", placeholder: "STX/sUSDT" },
      { key: "levels", label: "Grid Levels", type: "number", placeholder: "5" },
      { key: "spreadBps", label: "Spread (bps)", type: "number", placeholder: "30" },
      { key: "maxPositionPct", label: "Max Position (%)", type: "number", placeholder: "25" },
    ],
  },
  dca: {
    icon: Clock, label: "Dollar Cost Average", desc: "Auto-buy fixed amount at regular intervals.",
    defaults: { tokenIn: "STX", tokenOut: "sUSDT", amount: 1, intervalMinutes: 60 },
    fields: [
  ...
  sniper: {
    icon: Crosshair, label: "Token Sniper", desc: "Auto-buy new tokens matching filters.",
    defaults: { watchTokens: "", maxBuyAmount: 1, slippageBps: 100 },
    fields: [
      { key: "watchTokens", label: "Watch Tokens (comma)", type: "text", placeholder: "ALEX,WELSH" },
      { key: "maxBuyAmount", label: "Max Buy (STX)", type: "number", placeholder: "1", step: "0.1" },
      { key: "slippageBps", label: "Slippage (bps)", type: "number", placeholder: "100" },
    ],
  },
  copy: {
    icon: Copy, label: "Copy Trading", desc: "Mirror trades from a watched wallet.",
    defaults: { targetAddress: "", maxPerTrade: 10, minLiquidity: 1000 },
    fields: [
      { key: "targetAddress", label: "Target Address", type: "text", placeholder: "SP..." },
      { key: "maxPerTrade", label: "Max Per Trade (STX)", type: "number", placeholder: "10", step: "1" },
      { key: "minLiquidity", label: "Min Liquidity ($)", type: "number", placeholder: "1000" },
    ],
  },
};
