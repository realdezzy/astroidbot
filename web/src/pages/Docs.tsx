...
export function Docs() {
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-gray-100 selection:bg-brand-500/30 overflow-x-hidden font-sans flex flex-col">
      ...
      <header className="sticky top-0 z-50 backdrop-blur-md bg-gray-950/80 border-b border-gray-900">
        <div className="w-full max-w-full px-6 h-16 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <Link to="/" className="font-bold text-white text-lg tracking-tight hover:text-brand-400 transition-colors">AstroidBot</Link>
              <span className="text-xs block text-gray-500 -mt-1 font-mono">DOCUMENTATION</span>
            </div>
          </div>

          <div className="flex items-center gap-4">
            <Link to="/" className="text-sm font-medium text-gray-400 hover:text-white transition-colors flex items-center gap-1.5 py-1">
            ...
            <button
              ...
              className="lg:hidden p-2 text-gray-400 hover:text-white rounded-lg bg-gray-900 border border-gray-800 transition-colors"
            >
      ...
      <div className="flex-1 w-full max-w-full px-6 py-8 flex flex-col lg:flex-row gap-8 relative">
        ...
        <aside className="hidden lg:block w-72 shrink-0 space-y-6">
          {/* Search Panel */}
          <div className="bg-gray-900/50 backdrop-blur border border-gray-900 rounded-2xl p-4 relative">
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-9 pr-4 py-2 bg-gray-950 border border-gray-800 rounded-xl text-sm placeholder-gray-500 focus:border-brand-500 focus:outline-none text-white transition-colors"
              />
            </div>

            {/* Search Results Dropdown */}
            {searchQuery.trim() !== "" && (
              <div className="absolute left-0 right-0 mt-2 bg-gray-900 border border-gray-800 rounded-xl shadow-2xl z-30 max-h-80 overflow-y-auto p-2 space-y-1">
                <div className="text-[10px] uppercase tracking-wider text-gray-500 font-bold px-3 py-1">
                ...
                {searchResults.map((result) => (
                  <Link
                    ...
                    className="block p-2 rounded-lg hover:bg-gray-800/50 transition-colors text-left"
                  >
          ...
          {/* Grouped Links */}
          <div className="bg-gray-900/50 backdrop-blur border border-gray-900 rounded-2xl p-4 space-y-4">
              ...
              <nav className="space-y-4">
                {Object.entries(groupedDocs).map(([category, items]) => (
                  <div key={category} className="space-y-1">
                    <h4 className="text-xs font-bold text-gray-500 uppercase tracking-wider px-2 py-1 select-none">
                      {category}
                    </h4>
        ...
        {mobileMenuOpen && (
          <div className="lg:hidden fixed inset-0 z-40 bg-gray-950/95 backdrop-blur-md pt-20 px-6 overflow-y-auto">
            {/* Search Panel Mobile */}
            <div className="relative mb-6">
              ...
              <input
                ...
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
                className="w-full pl-9 pr-4 py-2 bg-gray-900 border border-gray-800 rounded-xl text-sm placeholder-gray-500 focus:border-brand-500 focus:outline-none text-white"
              />
              {/* Search Results Mobile */}
              {searchQuery.trim() !== "" && (
                <div className="absolute left-0 right-0 mt-2 bg-gray-900 border border-gray-800 rounded-xl shadow-2xl z-50 max-h-60 overflow-y-auto p-2 space-y-1">
                  {searchResults.map((result) => (
                    <Link
                      key={result.slug}
                      to={`/docs/${result.slug}`}
                    ...
                    >
                      ...
                      <div className="text-xs text-gray-400 line-clamp-2">{result.snippet}</div>
                    </Link>
                  ))}
                </div>
              )}
        ...
        {/* Main Content Pane */}
        <main className="flex-1 min-w-0 bg-gray-900/30 border border-gray-900/60 rounded-2xl p-6 sm:p-8 backdrop-blur">
          {loadingContent ? (
            ...
            <div className="min-h-[400px] flex flex-col items-center justify-center text-center p-6 border border-dashed border-gray-800 rounded-2xl">
              <FileText className="w-12 h-12 text-gray-600 mb-3" />
              <h3 className="text-lg font-bold text-white mb-1">Doc Not Found</h3>
        ...
        {toc.length > 0 && !loadingContent && (
          <aside className="hidden xl:block w-56 shrink-0">
            <div className="sticky top-24 space-y-4">
              ...
              <nav className="space-y-2 border-l border-gray-900 pl-2">
                {toc.map((heading) => (
                  <a
                    key={heading.id}
                    href={`#${heading.id}`}
                    ...
                    className={`block text-xs text-gray-400 hover:text-brand-400 transition-colors py-0.5 truncate ${
                      heading.level === 3 ? "pl-3 text-gray-500" : ""
                    }`}
                  >
                    {heading.text}
                  </a>
