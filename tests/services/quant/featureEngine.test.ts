...
vi.mock("../../../src/services/quant/candleService.js", () => ({
  CandleService: {
    getInstance: () => ({ getCandles: mockGetCandles }),
  },
}));
...
function candlesFromPrices(prices: number[]) {
  return prices.map((p) => ({ open: p, high: p, low: p, close: p, volume: 1000, timestamp: new Date() }));
}
...
describe("FeatureEngine", () => {
  ...
  it("computes positive return1h when price trended up over last 60 periods", async () => {
    const prices = linearPrices(300, 1.0, 0.01);
    mockGetCandles.mockResolvedValue(candlesFromPrices(prices));
    const features = await engine.compute("STX");
  ...
  it("computes positive ATR on a volatile series", async () => {
    const volatile = [1.0, 1.1, 0.9, 1.2, 0.85, 1.3, 0.8];
    // Pad to 30+ candles (minimum required)
    const prices = [...volatile, ...Array(23).fill(1.0)];
    mockGetCandles.mockResolvedValue(candlesFromPrices(prices));
    const features = await engine.compute("STX");
  ...
  it("computes non-zero Bollinger width when prices deviate", async () => {
    const deviating = [1.0, 1.5, 0.5, 2.0, 0.3, 1.8, 0.7, 1.9, 0.4, 2.1, 0.6, 1.7, 0.8, 1.6, 0.9, 1.4, 1.0, 1.3, 1.1, 1.2];
    // Pad to 30+ candles
    const prices = [...deviating, ...Array(10).fill(1.0)];
    mockGetCandles.mockResolvedValue(candlesFromPrices(prices));
    const features = await engine.compute("STX");
